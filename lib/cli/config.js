  ...
  if (action === 'edit') {
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        ...
        choices: getAllModels().map((model) => ({
          name: `${model.label || model.name} [${model.engine}/${model.lane}]`,
          value: model.id
        })),
        default: currentConfig.preferences?.defaultModel || currentConfig.engines?.claude?.model || 'sonnet'
      },
      {
        type: 'list',
        name: 'installMethod',
        message: 'Preferred runtime install method:',
        choices: ['npm', 'manual'],
        default: currentConfig.runtimes?.preferredInstallMethod || 'npm'
      },
      {
        type: 'confirm',
        name: 'checkOnBoot',
        message: 'Re-check runtimes on boot?',
        default: currentConfig.engines?.check_on_boot ?? false
      },
      {
        type: 'confirm',
        name: 'wakeLock',
    ...
    setConfigValue('preferences.defaultModel', answers.defaultModel);
    setConfigValue('runtimes.preferredInstallMethod', answers.installMethod);
    setConfigValue('engines.check_on_boot', answers.checkOnBoot);
