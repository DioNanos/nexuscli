...
describe('Database Integration', () => {
  ...
  test('should have runtime-aware columns on sessions and messages', async () => {
    const sessionCols = prepare("PRAGMA table_info(sessions)").all().map((row) => row.name);
    const messageCols = prepare("PRAGMA table_info(messages)").all().map((row) => row.name);

    expect(sessionCols).toEqual(expect.arrayContaining(['lane', 'runtime_id', 'provider_id', 'model_id']));
    expect(messageCols).toEqual(expect.arrayContaining(['lane', 'runtime_id', 'provider_id', 'model_id']));
  });
...
describe('API Routes Availability', () => {
  ...
  test('should have runtimes router module', () => {
    expect(() => require('../routes/runtimes')).not.toThrow();
  });
});
