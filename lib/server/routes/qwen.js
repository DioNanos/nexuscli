...
router.post('/', async (req, res) => {
  const fallbackModel = req.body?.model || 'qwen3-coder-plus';
  try {
    console.log('[Qwen] === NEW QWEN REQUEST ===');
    console.log('[Qwen] Body:', JSON.stringify(req.body, null, 2));

    const { conversationId, message, model = 'qwen3-coder-plus', lane, runtimeId, workspace, attachments } = req.body;
    ...
    const isAvailable = await qwenWrapper.isAvailable(runtimeSelection.command);
    if (!isAvailable) {
      return res.status(503).json({
        error: 'Qwen CLI not available',
        details: 'Please install Qwen CLI: npm install -g @qwen-code/qwen-code@latest'
      });
    }
