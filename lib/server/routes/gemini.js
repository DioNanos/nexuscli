...
router.post('/', async (req, res) => {
  try {
    ...
    const { conversationId, message, model = 'gemini-3-pro-preview', lane, runtimeId, workspace } = req.body;
    ...
    const isAvailable = await geminiWrapper.isAvailable(runtimeSelection.command);
    if (!isAvailable) {
