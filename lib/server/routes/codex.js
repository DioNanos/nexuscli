...
router.post('/', async (req, res) => {
  try {
    ...
    const { conversationId, message, model = 'gpt-5.3-codex', lane, runtimeId, reasoningEffort, workspace } = req.body;
    const runtimeSelection = runtimeManager.resolveRuntimeSelection({
      engine: 'codex',
      lane,
      runtimeId,
      modelId: model,
    ...
    const runtimeMeta = {
      lane: runtimeSelection.lane,
      runtimeId: runtimeSelection.runtimeId,
      providerId: runtimeSelection.model?.providerId || null,
      modelId: runtimeSelection.model?.id || model,
    };
    ...
    const isAvailable = await codexWrapper.isAvailable(runtimeSelection.command);
    if (!isAvailable) {
      return res.status(503).json({ error: 'Codex CLI not available' });
    }

    const hasExec = await codexWrapper.hasExecSupport(runtimeSelection.command);
    if (!hasExec) {
    ...
    try {
      ...
      const result = await codexWrapper.sendMessage({
        ...
        reasoningEffort,
        runtimeCommand: runtimeSelection.command,
        envOverrides: runtimeSelection.env,
        configOverrides: runtimeSelection.configOverrides,
        providerAuth: runtimeSelection.providerAuth,
        workspacePath,
...
router.get('/status', async (req, res) => {
  try {
    const runtimeSelection = runtimeManager.resolveRuntimeSelection({
      engine: 'codex',
      lane: req.query.lane,
      runtimeId: req.query.runtimeId,
      modelId: req.query.model,
    });
    const isAvailable = await codexWrapper.isAvailable(runtimeSelection.command);
    const hasExec = isAvailable ? await codexWrapper.hasExecSupport(runtimeSelection.command) : false;
