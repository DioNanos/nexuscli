...
// Dangerous commands that could harm the server or system
              ...
              if (event.type === 'status' && event.message) {
                ...
                if (msg.startsWith('Shell:') || msg.includes('Shell:')) {
                  ...
                  if (this._isDangerousCommand(shellCmd)) {
                    console.warn(`[GeminiWrapper] ⚠️  DANGEROUS COMMAND DETECTED: ${shellCmd.substring(0, 100)}`);
                    console.warn(`[GeminiWrapper] ⚠️  This command could harm the server or system!`);
                  }
                }
              }
