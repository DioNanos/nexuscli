...
class ClaudeWrapper extends BaseCliWrapper {
   ...
   */
  async sendMessage({ prompt, conversationId, model = 'sonnet', workspacePath, onStatus, runtimeCommand, envOverrides = {} }) {
    return new Promise((resolve, reject) => {
      ...
      const glmModelName = model === 'glm-5' ? 'GLM-5' : model === 'glm-4.7' ? 'GLM-4.7' : null;
      const isGLM = Boolean(glmModelName);
      ...
      if (isDeepSeek) {
        ...
        if (!glmKey) {
          const errorMsg = `Z.ai API key not configured for ${glmModelName || 'GLM'}!\n\n` +
            `Run this command to add your API key:\n` +
        ...
        // GLM custom models use Z.ai Anthropic-compatible API
        spawnEnv.ANTHROPIC_BASE_URL = 'https://api.z.ai/api/anthropic';
        spawnEnv.ANTHROPIC_AUTH_TOKEN = glmKey;
        spawnEnv.ANTHROPIC_MODEL = glmModelName;
        spawnEnv.API_TIMEOUT_MS = '3000000'; // 50 minutes timeout
        console.log(`[ClaudeWrapper] ${glmModelName} detected - using Z.ai API with extended timeout`);
      }
      ...
      if (isGLM) {
        timeoutMs = 3600000; // 60 minutes for GLM models (slow responses)
